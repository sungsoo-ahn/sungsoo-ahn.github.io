...
This post uses controlled Gaussian work ensembles rather than a full steered
MD integrator. That is intentional for the hidden draft. It isolates the
statistical identity: work values are path-ensemble data, and the free-energy
estimate is not the mean work. A final production MD version should add real
steered trajectories and keep the same diagnostic language.
...
the path ensemble with the correct weighting.

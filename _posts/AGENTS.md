...
**Skills:** `/blog-writing` for prose style, `/lecture-adaptation` for source-faithful lecture work, and `/jekyll-writing` for rendering rules.
...
For lecture-derived revisions, use `/blog-writing` for general tutorial depth and
`/lecture-adaptation` whenever a source deck is authoritative. The latter owns
slide coverage, source fidelity, and PPTX-native extraction rules; do not
duplicate that workflow in this directory guide.

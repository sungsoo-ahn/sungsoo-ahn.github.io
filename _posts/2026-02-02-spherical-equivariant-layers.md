...
[^physics-sh]: From a physics standpoint, spherical harmonics arise naturally as the "right" basis for rotations. In quantum mechanics, angular momentum operators generate rotations, and spherical harmonics are their eigenstates—the functions that respond to these operators in the simplest possible way (just getting multiplied by a number). This is why their transformation under rotation is so well-behaved: they are the natural basis in which rotations act cleanly.

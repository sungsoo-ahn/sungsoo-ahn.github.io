...
[^physics-sh]: From a physics standpoint, angular momentum operators are the *generators* of rotations—infinitesimal rotations are expressed as angular momentum operators, and finite rotations are obtained by exponentiation. Spherical harmonics are simultaneous eigenstates of the total angular momentum operator $L^2$ (with eigenvalue $\ell(\ell+1)$) and the $z$-component $L_z$ (with eigenvalue $m$). Because they diagonalize the rotation generators, their transformation under any finite rotation follows directly from this eigenvalue structure, yielding the Wigner-D matrices.

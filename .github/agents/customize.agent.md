...
**Development (local testing for this repository):**
...
1. **Run locally with Homebrew Ruby**:
...
2. **Run content validation** – For blog changes, run `python3 scripts/validate_blog.py`.

3. **Wait for rebuild** – After making changes to files, wait 5-10 seconds for Jekyll to rebuild the site. You'll see output in the terminal indicating the rebuild is complete.

4. **Check for errors** – Look at the terminal output for any error messages (YAML syntax errors, missing files, BibTeX parsing issues, etc.).

5. **Verify visually** – Manually navigate through your site:
...
6. **Test on different pages** – If you modified:
...
- Local preview with Homebrew Ruby runs on `http://localhost:4000`
